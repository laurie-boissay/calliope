...
def arguments_reroll(cmd_text):
	"""
	Sépare la commande en paramètres de commande.

	Renvoit un cmd_text d'erreur si les paramètres obligatoires
	...
	cmd_text = list(cmd_text.split(","))

	type_perso = cmd_text[0].strip("!")

	try :
		nb_points_de_carac = cmd_text[1]	
		valeur_max = cmd_text[2]
	except IndexError:
		text = [
	...
	for i in range(3, len(cmd_text), 1):
		cmd_text[i] = cmd_text[i].strip(" ")

	genre_proscrit, races_proscrites, metiers_proscrits = découpage_en_arguments(cmd_text)
	
	return reroll(nb_points_de_carac, valeur_max, genre_proscrit, races_proscrites, metiers_proscrits, **arguments)


def découpage_en_arguments(cmd_text):
	"""
	Prend les paramètres listés dans cmd_text 
...
def reroll(nb_points_de_carac, valeur_max, genre_proscrit, races_proscrites, metiers_proscrits, **kwargs):
	...
	un cmd_text d'erreur si ce n'est pas le cas.
...
def make_int(number):
	...
	Renvoit un cmd_text d'erreur si le nombre ne peut être
	transformé.
